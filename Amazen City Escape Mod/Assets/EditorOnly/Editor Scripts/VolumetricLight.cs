...
public class VolumetricLight : MonoBehaviour
{
	...
	private void VolumetricLightRenderer_PreRenderEvent(VolumetricLightRenderer renderer, Matrix4x4 viewProj)
	{
		...
		if (_light.gameObject.activeInHierarchy && _light.enabled)
		{
			...
			_material.SetVector("_MieG", new Vector4(1f - MieG * MieG, 1f + MieG * MieG, 2f * MieG, 0.07957747f));
	...
	private void SetupSpotLight(VolumetricLightRenderer renderer, Matrix4x4 viewProj)
	{
		...
		Matrix4x4 rhs = Matrix4x4.Perspective(_light.spotAngle, 1f, 0f, 1f);
		_material.SetMatrix("_MyLightMatrix0", lhs * rhs * inverse);
		_material.SetMatrix("_WorldViewProj", viewProj * matrix4x);
		_material.SetVector("_LightPos", new Vector4(_light.transform.position.x, _light.transform.position.y, _light.transform.position.z, 1f / (_light.range * _light.range)));
		...
		if (_light.shadows != 0 && !flag)
		{
			...
			rhs = ((!_reversedZ) ? Matrix4x4.Perspective(_light.spotAngle, 1f, _light.shadowNearPlane, _light.range) : Matrix4x4.Perspective(_light.spotAngle, 1f, _light.range, _light.shadowNearPlane));
			Matrix4x4 lhs2 = lhs * rhs;
			lhs2[0, 2] *= -1f;
			lhs2[1, 2] *= -1f;
			lhs2[2, 2] *= -1f;
			lhs2[3, 2] *= -1f;
			_material.SetMatrix("_MyWorld2Shadow", lhs2 * inverse);
			_material.SetMatrix("_WorldView", lhs2 * inverse);
